...
app.use(
  cors({
    origin: process.env.CORS_ORIGIN || "https://lmks.netlify.app", // Configurable CORS origin
  ...
  const uploadNewFile = () => {
    ...
    uploadFile(
      ...
      (result) => {
        ...
        sendNotification(className); // Send notification
        res.send(result);
      },
      (error) => {
...
const sendNotification = (className) => {
  const message = {
    notification: {
      title: 'New Timetable Available',
    body: `A new timetable has been uploaded for ${className}. Please check it out.`
    },
    topic: className // Send notification to specific class topic
  };
  ...
  const uploadNewFile = () => {
    ...
    uploadFile(
      ...
      (result) => {
        ...
        examNotification(className); // Send notification
...
// Function to send notification
const examNotification = (className) => {
  const message = {
    notification: {
      title: 'New Timetable Available',
      body: `A new Exam-Timetable has been uploaded for ${className}. Please check it out.`
    },
    topic: className // Send notification to specific class topic
  };
  ...
  firebaseAdmin.messaging().send(message)
    ...
    .catch(error => {
      ...
      const sheetId = process.env.DATA_SHEET_ID;
...
// Update
app.put('/update-post/:index', async (req, res) => {
  ...
  try {
    ...
    rows[index] = [title, description];

    const updateResponse = await sheets.spreadsheets.values.update({
      spreadsheetId: sheetId,
      range: `Sheet1!A${parseInt(index, 10) + 1}:B${parseInt(index, 10) + 1}`,
      valueInputOption: 'RAW',
      requestBody: {
        values: [rows[index]],
      },
